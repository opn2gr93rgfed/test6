# SMART DYNAMIC ONE TIME PROVIDER - ОДНОРАЗОВЫЕ ПРОФИЛИ
"""
Provider: smart_dynamic_onetime
Генератор скриптов с динамичной системой поиска ответов + АВТОУДАЛЕНИЕ ПРОФИЛЕЙ

🔥 ОТЛИЧИЯ ОТ smart_dynamic:
- После завершения скрипта профиль АВТОМАТИЧЕСКИ УДАЛЯЕТСЯ через API
- Идеально для массовых задач где не нужно сохранять профили
- Экономия места и очистка аккаунта от мусорных профилей
...
4. 🔥 В конце: УДАЛЯЕТ профиль через delete_profile()
...
- АВТОУДАЛЕНИЕ профилей после завершения работы
"""
